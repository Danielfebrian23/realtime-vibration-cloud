...
if TELEGRAM_AVAILABLE:
    ...
    async def error_handler(update: object, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if "Conflict" in error_msg or "terminated by other getUpdates" in error_msg:
            print("Bot conflict detected. Stopping Telegram bot and continuing with Flask only.")
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--no-telegram':
        use_telegram = False
        print("Running without Telegram bot (--no-telegram flag)")
    ...
    # Jalankan Telegram bot jika diminta dan tersedia
    if use_telegram and TELEGRAM_AVAILABLE:
        try:
            print("Starting Telegram bot...")
            success = main_telegram()
            if not success:
                print("Telegram bot failed. Continuing with Flask server only...")
