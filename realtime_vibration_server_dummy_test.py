...
def load_trained_models():
    ...
    try:
        print("Training new models for DUMMY DATA testing...")
...
def train_models_from_data():
    """Train models from Dataset_Bersih.xlsx (raw X, Y, Z data)"""
    global iso_forest_model, pca_model, scaler_model
    try:
        # Load training data from Dataset_Bersih.xlsx (raw data)
        print("Loading Dataset_Bersih.xlsx for training...")
        df_original = pd.read_excel("Dataset_Bersih.xlsx")
        
        print(f"Original columns: {df_original.columns.tolist()}")
        print(f"Column names with repr: {[repr(col) for col in df_original.columns]}")
        ...
        if x_col and y_col and z_col:
            print(f"Found columns: X='{x_col}', Y='{y_col}', Z='{z_col}'")
            raw_features = df_original[[x_col, y_col, z_col]].dropna()
        else:
            print("Could not find X, Y, Z columns. Using fallback...")
            raise Exception("X, Y, Z columns not found")
        
        print(f"Raw features shape: {raw_features.shape}")
        print(f"Sample data:\n{raw_features.head()}")
        
        # Train PCA and Scaler
        scaler_model = StandardScaler()
        X_scaled = scaler_model.fit_transform(raw_features)
        pca_model = PCA(n_components=2)
        pca_features = pca_model.fit_transform(X_scaled)
        
        # Train Isolation Forest on PCA features (2 features)
        iso_forest_model = IsolationForest(contamination='auto', random_state=42)
        iso_forest_model.fit(pca_features)  # Use PCA features (2D)
        
        print("Models trained successfully with raw data!")
        print(f"Training samples: {len(raw_features)}")
        print(f"PCA explained variance: {pca_model.explained_variance_ratio_}")
        
    except Exception as e:
        print(f"Error training models: {e}")
        print("Using fallback models...")
        # Fallback: create simple models with dummy data
        dummy_data = np.random.randn(100, 2)  # 100 samples, 2 features (PC1, PC2)
        scaler_model = StandardScaler()
        scaler_model.fit(dummy_data)
        pca_model = PCA(n_components=2)
        pca_model.fit(dummy_data)
        iso_forest_model = IsolationForest(contamination=0.1, random_state=42)
        iso_forest_model.fit(dummy_data)  # Train on 2D data
...
def extract_features_from_buffer(data_buffer):
    ...
    if scaler_model and pca_model and hasattr(scaler_model, 'mean_'):
        try:
            ...
            # Fallback: use RMS values as features
            features['PC1'] = features['rms_x']
            features['PC2'] = features['rms_y']
    else:
        # Fallback: use RMS values as features
        features['PC1'] = features['rms_x']
        features['PC2'] = features['rms_y']
...
def classify_vibration(features):
    ...
    if not is_model_loaded or iso_forest_model is None:
        return "UNKNOWN", 0.0
    try:
        # Prepare features for prediction (PC1, PC2 only)
        feature_vector = np.array([
            features['PC1'], features['PC2']
        ]).reshape(1, -1)
        
        print(f"Feature vector shape: {feature_vector.shape}")
        print(f"Feature values: PC1={features['PC1']:.3f}, PC2={features['PC2']:.3f}")
        ...
        print(f"Anomaly score: {anomaly_score:.3f}, Is anomaly: {is_anomaly}")
        ...
        print(f"Classification result: {severity} (confidence: {confidence:.3f})")
...
def predict_vibration():
    ...
    try:
        ...
        # Ambil data X, Y, Z terakhir yang masuk
        last_x = float(x_data[-1]) if len(x_data) > 0 else None
        last_y = float(y_data[-1]) if len(y_data) > 0 else None
        last_z = float(z_data[-1]) if len(z_data) > 0 else None
...
if __name__ == '__main__':
    ...
    def run_flask():
        print("Starting Flask server for DUMMY DATA testing...")
