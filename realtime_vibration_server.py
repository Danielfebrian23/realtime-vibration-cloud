...
import io
import requests  # Tambahkan untuk HTTP request ke ESP32
...
# Simpan status terakhir (bisa diganti dengan database/file sesuai kebutuhan)
last_status = {
    'severity': 'NORMAL',
    'confidence': 0.98,
    ...
    'riwayat': [("NORMAL", 0.98), ("RINGAN", 0.6), ("BERAT", 0.2)]  # Contoh data
}
...
AUTHORIZED_USER_ID = 6080177529  # Ganti dengan ID kamu

# Ganti sesuai IP ESP32 kamu 
ESP32_IP = '192.168.43.7'  # TODO: Ganti dengan IP ESP32 di jaringanmu
ESP32_HTTP_PORT = 80  # Biasanya 80, ganti jika berbeda

# Fungsi untuk kirim perintah ke ESP32 via HTTP
def send_http_command(command):
    try:
        url = f"http://{ESP32_IP}:{ESP32_HTTP_PORT}/{command}"
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id != AUTHORIZED_USER_ID:
        ...
        return
    measuring_status['active'] = True
    send_http_command('start')  # Kirim ke ESP32 via HTTP
    await update.message.reply_text("Pengukuran real-time DIMULAI. Sistem siap menerima data dari ESP32.")
...
async def stop(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    send_http_command('stop')  # Kirim ke ESP32 via HTTP
    await update.message.reply_text("Pengukuran real-time DIHENTIKAN. Sistem tidak akan memproses data dari ESP32.")
...
async def cek(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"Kondisi Motor: {status}\nConfidence: {confidence}")
...
async def penjelasan(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Ambil penjelasan berdasarkan kondisi
    if current_severity in PENJELASAN_BY_CONDITION:
        explanation_message = PENJELASAN_BY_CONDITION[current_severity]
    else:
        explanation_message = PENJELASAN_BY_CONDITION['NORMAL']  # Default ke normal
    
    # Kirim pesan dengan format yang informatif
...
async def grafik(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    plt.title("Riwayat Anomali Motor")
    plt.xlabel("Waktu")
...
def main_telegram():
    try:
        ...
        # Initialize the bot properly (no need to await in this context)
...
def load_trained_models():
    ...
    try:
        # Load models (jika sudah ada file trained models)
        # iso_forest_model = joblib.load('isolation_forest_model.pkl')
        # pca_model = joblib.load('pca_model.pkl')
        # scaler_model = joblib.load('scaler_model.pkl')
        # Untuk sementara, kita akan train model baru setiap kali server start
...
if __name__ == '__main__':
    ...
    # Jalankan Telegram bot di main thread (WAJIB untuk python-telegram-bot v20+)
    try:
        ...
        # Keep Flask running even if Telegram bot fails
        try:
            while True:
                time.sleep(1)
        except KeyboardInterrupt:
